namespace loop_helper {
	...
	public:
		...
		virtual t_size get_more_chunk(audio_chunk & p_chunk, mem_block_container * p_raw, abort_callback & p_abort, t_size candidate) {
			...
			mem_block_container_impl * ptmp_block = NULL;
			if (p_raw != NULL) ptmp_block = &tmp_block;
	...
	public:
		...
		loop_event_point_baseimpl() : flags(on_looping) {}
		loop_event_point_baseimpl(unsigned p_flags) : flags(p_flags) {}
		enum {
			on_looping    = 1 << 0,
			on_no_looping = 1 << 1,
		};
	...
	public:
		...
		loop_event_point_simple() : from(0), to(0), maxrepeats(0), repeats(0), loop_event_point_baseimpl(on_looping) {}
		t_uint64 from, to;
	...
	public:
		...
		loop_event_point_end() : position(0), loop_event_point_baseimpl(on_no_looping) {}
		t_uint64 position;
	...
	public:
		...
		virtual bool get_no_looping() const {return m_no_looping || cfg_loop_disable.get();}
		virtual void set_info_prefix(const char *p_prefix) {m_info_prefix = p_prefix;}

		virtual bool open_path(file::ptr p_filehint,const char * path,t_input_open_reason p_reason,abort_callback & p_abort,bool p_from_redirect,bool p_skip_hints) {
		...
		virtual bool process_event(loop_event_point::ptr point, t_uint64 p_start, audio_chunk & p_chunk, mem_block_container * p_raw, abort_callback & p_abort) {
			return point->process(this, p_start, p_chunk, p_raw, p_abort);
		}

		virtual bool process_event(loop_event_point::ptr point, abort_callback & p_abort) {
	...
	public:
		...
		void retag_commit(abort_callback & p_abort) {throw exception_io_unsupported_format();}
